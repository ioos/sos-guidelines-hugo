...
| featureOfInterest | 0…* | Specifies the feature for which observations are requested. This should be presented in a form of comma-separated URIs advertised in the GetCapabilities document or a simple bounding box for requested data.  <br><br>Since SOS v1.0 does not specify the exact format for spatial filtering, the IOOS Convention follows the early Ocean Science Interoperability Best Practice recommendation: `featureOfInterest=BBOX:min_lon,min_lat,max_lon,max_lat`. <br><br>SOS v2.0 specifies different spatial filtering format: `featureOfInterest=om:featureOfInterest/*/sams:shape,min_lat,min_lon,max_lat,max_lon,http://www.opengis.net/def/crs/EPSG/0/4326`. <br><br>Early versions of i52N IOOS SOS servers supported only SOS v2.0 format; starting with i52N v1.0.1 both formats are supported. See GetObservation request examples below for details. |
